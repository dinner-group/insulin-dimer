...
inv_rate_arr = np.zeros((len(k_ls), len(rn_ls), len(lag_ls), len(mem_ls)))
for i_rn, rn in enumerate(rn_ls): #0, 1, 2
    for i_k, k in enumerate(k_ls): #100, 200, 400, 600
        st_uq, st_id, st_inv = np.unique(state_arr[i_k,i_rn].reshape(ntraj, length), return_index=True, return_inverse=True)
        t0 = time()
        for i_mem, mem in enumerate(mem_ls): #1, 4, 2, -4            
            for i_lag, lag in enumerate(lag_ls): #100, 200, 400, 600, 800, 900
                ...
                for i_st, q_val in enumerate(qf_unq[i_k][i_rn, i_lag, i_mem]):
                    qf[st_inv==i_st] = q_val
                qf = qf.reshape(ntraj, length)
                inv_rate_arr[i_k, i_rn, i_lag, i_mem] = 1/(extq2.tpt.rate(qf, 1-qf, w_us, inD, qf, lag)/(dt*log))

                print(f"Done: k={k}, rn={rn}, mem={mem}, lag={lag}")
        #np.save(f"{workdir}/step4_rate/mem/inv_rate_arr.npy", inv_rate_arr)    
        np.save(f"{workdir}/step4_rate/mem/inv_rate_arr_hk.npy", inv_rate_arr)    
