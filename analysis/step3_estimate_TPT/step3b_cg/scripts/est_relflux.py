...
for key, tmp in stid_cg.items():
    if 'unzipped' in key:
        key_z = key.split("_")[0]
        if '_up' in key:
            state_id_set_1.append(np.hstack((tmp, stid_cg[f"{key_z}_down"])))
    else:
        state_id_set_1.append(tmp)

state_id_set = state_id_set_1
"""
#4
state_id_set = []
...
"""
outfname = f"{workdir}/step7_structure/output/step4_atpt/cg_flux1.npy"
